  ...
  return (
    <div className="flex flex-row bg-white  p-1 mx-4 justify-around  border-black border-b-2">
      <div className="flex flex-col">
        <div className="text-center text-2xl underline">{item.name}</div>
        <div>
      ...
      <div className="my-auto flex flex-row justify-evenly">
        {item.qty > 1 ? (
          <MinusIcon
            onClick={() => {
              dispatch(strictDecreaseQty({ name: item.name, qty: item.qty }));
            }}
        ...
        <PlusIcon
          ...
          className="h-14 w-14 p-1 mr-16 hover:cursor-pointer hover:fill-green-500"
        />
      </div>
    </div>
  );
...
const ViewCartScreen = () => {
  ...
  const EmptyCart = () => {
    return (
      <div className="flex flex-col my-auto p-3 text-center">
        <div className="text-3xl font-playfair">
  ...
  return (
    <div className="flex flex-col bg-color-light mx-60  pt-6">
      <div className="text-center text-7xl  mb-16 underline tracking-wider font-bold font-poppins">
        Your Cart
      </div>
      {anythingInCart ? (
        cartItems.map((item) => <CartScreenListComponent item={item} />)
      ) : (
        <EmptyCart />
      )}
      <div>
        <div className="text-end text-4xl p-1 mr-4 mt-10">Total:</div>
        <div className="text-end text-4xl p-2 mr-4">$ {getCartSubtotal()}</div>
